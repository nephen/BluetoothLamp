...
const atts_svc_desc_t blps_bps_svc = ATT_SVC_BLOOD_PRESSURE;

/// Blood Pressure Sensor Service - Blood Pressure Measurement Characteristic
const struct atts_char_desc blps_bp_meas_char = ATTS_CHAR(ATT_CHAR_PROP_IND,
...
const struct atts_char_desc blps_interm_cuff_press_char = ATTS_CHAR(ATT_CHAR_PROP_NTF,
                                                                    0, //No attribute table for BPS
                                                                    ATT_CHAR_INTERMEDIATE_CUFF_PRESSURE);

/// Blood Pressure Sensor Service - Blood Pressure Feature Characteristic
const struct atts_char_desc blps_bp_feature_char = ATTS_CHAR(ATT_CHAR_PROP_RD,
                                                             0, //No attribute table for BPS
...
void blps_meas_send_cfm_send(uint8_t status)
{
    ...
    cfm->conhdl     = blps_env.con_info.conhdl;
    cfm->status     = status;

    ke_msg_send(cfm);
}

void blps_disable(void)
{
	//Disable BPS in database
    attsdb_svc_set_permission(blps_env.shdl, PERM_RIGHT_DISABLE);
