...
const struct atts_desc diss_att_db[DIS_IDX_NB] =
{
...
const atts_svc_desc_t diss_dis_svc = ATT_SVC_DEVICE_INFO;

const struct atts_char_desc diss_manufacturer_name_char = ATTS_CHAR(ATT_CHAR_PROP_RD, DIS_MANUFACTURER_NAME_CHAR,
                                                                                      ATT_CHAR_MANUF_NAME);
const struct atts_char_desc diss_model_nb_str_char      = ATTS_CHAR(ATT_CHAR_PROP_RD, DIS_MODEL_NB_STR_CHAR,
                                                                                      ATT_CHAR_MODEL_NB);
const struct atts_char_desc diss_serial_nb_str_char     = ATTS_CHAR(ATT_CHAR_PROP_RD, DIS_SERIAL_NB_STR_CHAR,
                                                                                      ATT_CHAR_SERIAL_NB);
const struct atts_char_desc diss_hard_rev_str_char      = ATTS_CHAR(ATT_CHAR_PROP_RD, DIS_HARD_REV_STR_CHAR,
                                                                                      ATT_CHAR_HW_REV);
const struct atts_char_desc diss_firm_rev_str_char      = ATTS_CHAR(ATT_CHAR_PROP_RD, DIS_FIRM_REV_STR_CHAR,
                                                                                      ATT_CHAR_FW_REV);
const struct atts_char_desc diss_sw_rev_str_char        = ATTS_CHAR(ATT_CHAR_PROP_RD, DIS_SW_REV_STR_CHAR,
                                                                                      ATT_CHAR_SW_REV);
const struct atts_char_desc diss_system_id_char         = ATTS_CHAR(ATT_CHAR_PROP_RD, DIS_SYSTEM_ID_CHAR,
                                                                                      ATT_CHAR_SYS_ID);
const struct atts_char_desc diss_ieee_char              = ATTS_CHAR(ATT_CHAR_PROP_RD, DIS_IEEE_CHAR,
                                                                                      ATT_CHAR_IEEE_CERTIF);
const struct atts_char_desc diss_pnp_id_char            = ATTS_CHAR(ATT_CHAR_PROP_RD, DIS_PNP_ID_CHAR,
                                                                                      ATT_CHAR_PNP_ID);
...
void diss_init(void)
{
	...
	// Register DISS task into kernel
    task_diss_desc_register();
...
void diss_disable(void)
{
    ...
    ind->conhdl = diss_env.con_info.conhdl;

    ke_msg_send(ind);

    //Disable DIS in database
    attsdb_svc_set_permission(diss_env.shdl, PERM(SVC, DISABLE));
