                                      ...
                                      ke_task_id_t const src_id)
{
    ...
    status = atts_svc_create_db(&diss_env.shdl, (uint8_t *)&cfg_flag, DIS_IDX_NB, &diss_env.att_tbl[0],
                               dest_id, &diss_att_db[0]);

    if (status == ATT_ERR_NO_ERROR)
    {
        //Disable service
        status = attsdb_svc_set_permission(diss_env.shdl, PERM(SVC, DISABLE));
    ...
    if (param->char_code <= DIS_PNP_ID_CHAR)
    {
        ...
        if (handle != PRF_ERR_INEXISTENT_HDL)
        {
            ...
            if (status == PRF_ERR_OK)
            {
                ...
                attsdb_att_set_value(handle + 1, param->val_len, (uint8_t *)&param->val[0]);
            }
        }
        else
        {
                                   ...
                                   ke_task_id_t const src_id)
{
    ...
    diss_env.con_info.conhdl = param->conhdl;
    ...
    else
    {
        ...
        attsdb_svc_set_permission(diss_env.shdl, param->sec_lvl);
...
static int gap_discon_cmp_evt_handler(ke_msg_id_t const msgid,
                                      ...
                                      ke_task_id_t const src_id)
{
    if (param->conhdl == diss_env.con_info.conhdl)
    {
        diss_disable();
    }
...
ke_state_t diss_state[DISS_IDX_MAX];


// Register DISS task into kernel
void task_diss_desc_register(void)
{
    struct ke_task_desc  task_diss_desc;
    
    task_diss_desc.state_handler = diss_state_handler;
    task_diss_desc.default_handler=&diss_default_handler;
    task_diss_desc.state = diss_state;
    task_diss_desc.state_max = DISS_STATE_MAX;
    task_diss_desc.idx_max = DISS_IDX_MAX;

    task_desc_register(TASK_DISS, task_diss_desc);
}
