...
const struct atts_desc htpt_att_db[HTS_IDX_NB] =
{
...
const atts_svc_desc_t htpt_hlth_therm_svc = ATT_SVC_HEALTH_THERMOM;

/// Health Thermometer Service Temperature Measurement Characteristic
const struct atts_char_desc htpt_temp_meas_char = ATTS_CHAR(ATT_CHAR_PROP_IND, HTPT_TEMP_MEAS_CHAR,
                                                            ATT_CHAR_TEMPERATURE_MEAS);

/// Health Thermometer Service Temperature Type Characteristic
const struct atts_char_desc htpt_temp_type_char = ATTS_CHAR(ATT_CHAR_PROP_RD, HTPT_TEMP_TYPE_CHAR,
                                                            ATT_CHAR_TEMPERATURE_TYPE);

/// Health Thermometer Service Intermediate Temperature Characteristic
const struct atts_char_desc htpt_interm_temp_char = ATTS_CHAR(ATT_CHAR_PROP_NTF, HTPT_INTERM_TEMP_CHAR,
                                                              ATT_CHAR_INTERMED_TEMPERATURE);

/// Health Thermometer Service Measurement Interval Characteristic
const struct atts_char_desc htpt_meas_intv_char = ATTS_CHAR(ATT_CHAR_PROP_RD, HTPT_MEAS_INTV_CHAR,
...
void htpt_temp_send_cfm_send(uint8_t status, uint8_t cfm_type)
{
    ...
    cfm->conhdl     = htpt_env.con_info.conhdl;
    cfm->status     = status;
    cfm->cfm_type   = cfm_type;

    ke_msg_send(cfm);
}

void htpt_disable(void)
{
    atts_size_t att_length;
    uint8_t *att_value;

    //Disable HTS in database
    attsdb_svc_set_permission(htpt_env.shdl, PERM_RIGHT_DISABLE);
    ...
    if (HTPT_IS_CHAR_SUPPORTED(HTPT_MEAS_INTV_CHAR))
    {
        ...
        attsdb_att_get_value(htpt_env.shdl + htpt_env.att_tbl[HTPT_MEAS_INTV_CHAR] + 1,
