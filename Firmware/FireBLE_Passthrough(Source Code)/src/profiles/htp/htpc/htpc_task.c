                                             ...
                                             ke_task_id_t const src_id)
{
    ...
    if (param->status == CO_ERROR_NO_ERROR)
    {
        ...
        htpc_env->hts.svc.shdl = param->list[0].start_hdl;
        htpc_env->hts.svc.ehdl = param->list[0].end_hdl;
        htpc_env->nb_svc++;
    }
        ...
        if (htpc_env->hts.chars[HTPC_CHAR_HTS_MEAS_INTV].char_hdl != ATT_INVALID_SEARCH_HANDLE)
        {
                                  ...
                                  == ATT_CHAR_PROP_WR)
            {
                ...
                prf_gatt_write(&htpc_env->con_info, htpc_env->hts.chars[HTPC_CHAR_HTS_MEAS_INTV].val_hdl,
                               (uint8_t *)&param->intv, sizeof(uint16_t), GATT_WRITE_CHAR);
            }
                                        ...
                                        ke_task_id_t const src_id)
{
    ...
    if (param->conhdl == htpc_env->con_info.conhdl)
    {
        //Intermediate Temp Char.
        if (param->charhdl == htpc_env->hts.chars[HTPC_CHAR_HTS_INTM_TEMP].val_hdl)
        {
            htpc_unpack_temp(htpc_env, (uint8_t *)&(param->value), param->size, HTPC_TEMP_MEAS_INTM);
        }
    }
...
static int gap_discon_cmp_evt_handler(ke_msg_id_t const msgid,
                                      ...
                                      ke_task_id_t const src_id)
{
    PRF_CLIENT_DISABLE_IND_SEND(htpc_envs, dest_id, HTPC);
...
ke_state_t htpc_state[HTPC_IDX_MAX];

// Register HTPC task into kernel
void task_htpc_desc_register(void)
{
    struct ke_task_desc task_htpc_desc;
    
    task_htpc_desc.state_handler = htpc_state_handler;
    task_htpc_desc.default_handler=&htpc_default_handler;
    task_htpc_desc.state = htpc_state;
    task_htpc_desc.state_max = HTPC_STATE_MAX;
    task_htpc_desc.idx_max = HTPC_IDX_MAX;
        
    task_desc_register(TASK_HTPC, task_htpc_desc);
}
