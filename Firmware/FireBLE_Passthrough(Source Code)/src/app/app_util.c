...
uint8_t app_set_adv_data(uint16_t disc_mode)
{
...
#else
    nvds_tag_len_t name_length = ADV_DATA_LEN - strlen(MANU_LOGO) -5; // The maximum length of Advertising data is 31 Octets
 		//QPRINTF("\r\nfunc : %s name_length->%d\r\n",__func__,name_length);
    ...
    else
    {
		...
		if (name_length > ADV_DATA_LEN - strlen(MANU_LOGO) -5)
		{
			name_length = strlen(QN_LOCAL_NAME);
			strcpy((char *)&app_env.adv_data[5], QN_LOCAL_NAME);
		}
...
#ifdef	MANU_LOGO
		uint8_t manu_length = strlen(MANU_LOGO);
	
		app_env.adv_data[len + 0] = manu_length + 1;
